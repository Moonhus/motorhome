  ...
  return (
    <div>
      ...
      >
        ...
        <p className="pointer-events-none absolute bottom-3 right-3 rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-forest">
          {index + 1} of {count}
        </p>
        {count > 1 ? (
          <>
      ...
      {open ? (
        <div
          ...
          className="fixed inset-0 z-50 flex flex-col bg-forest-deep/95"
          onTouchStart={(event) => onTouchStart(event.touches[0].clientX)}
          onTouchEnd={(event) => onTouchEnd(event.changedTouches[0].clientX)}
        >
